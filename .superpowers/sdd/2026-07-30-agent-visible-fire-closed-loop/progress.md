...
Task 15: rereview-1 requested changes (Critical 0 / Important 1 / Minor 0): environment-assignment prefixes and real/prefixed systemd .service unit names bypassed the ai-service launch guard.
Task 15: fix round 2/5 implementation complete (1 addressed, 0 known open — normalized env/sudo/KEY=value prefixes and service-unit suffix/prefix matching; four exact bypass fixtures plus prohibition-prose negatives; 20 fixture scenarios and active checkout PASS). Product logic and release/device status remain unchanged.
Task 15: rereview-2 requested changes (Critical 0 / Important 1 / Minor 0): systemd template instance units such as ai-service@prod.service and uavfire-ai-service@prod.service bypassed the unit token.
Task 15: fix round 3/5 implementation complete (1 addressed, 0 known open — normalized optional uavfire prefix, legal @instance and optional .service suffix across start/restart/enable --now/try-restart; four instance fixtures plus prohibition-prose negatives; 24 fixture scenarios and active checkout PASS). Product logic and release/device status remain unchanged.
Task 15: rereview-3 requested changes (Critical 0 / Important 2 / Minor 0): underscore/arbitrary legal prefixes were missed, while lifecycle substrings in is-enabled/CanStart/CanRestart caused false positives.
Task 15: fix round 4/5 implementation complete (2 addressed, 0 known open — separator-bounded legal unit prefixes with ai[-_]service, exact lifecycle argument tokens, safe query/property and neighboring-token fixtures; 27 fixture scenarios and active checkout PASS). Product logic and release/device status remain unchanged.
Task 15: complete (final commit 5ec2e8a, independent rereview APPROVE; Critical 0 / Important 0 / Minor 0). All 27 static-policy fixtures and the active checkout pass. Automated evidence remains valid; RC Plus and all device/flight gates remain BLOCKED_PENDING_DEVICE, so release status remains NOT_READY_DEFAULT_OFF.
