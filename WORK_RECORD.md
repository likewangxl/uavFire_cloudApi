...
## 35. 2026-09-05 本地源码与设计资料补齐

按用户要求，将本地尚未进入主线的源文件和设计资料纳入版本管理：3 份 SVG 概念稿、3 个宣传生成脚本、宣传文案、5 份可编辑历史 PPT、22 张脚本输入原图、2 份历史推广设计/计划，以及旧工作区的 19 个标识源文件快照。原始文件共 55 个、59,408,725 字节，来源与 SHA-256 见 [归档清单](docs/local-source-archive-2026-09-05.json)。

- “燧巡天眼”作为独立方案保存在 `rcplus-msdk-agent/design/app-icon/variants/suixun-tianyan/`，当前应用标识和业务代码保持现状。
- 解除 `design/`、`docs/promo/` 的整目录忽略，改为排除可重建预览、裁剪图和背景图；补充 Android 包、Python/Kotlin 缓存及本地训练权重的忽略规则。
- 安装包、发布压缩包、模型权重、视频、依赖和构建产物继续留在本地。此前已经跟踪的运行资源不在本次改动范围内。
