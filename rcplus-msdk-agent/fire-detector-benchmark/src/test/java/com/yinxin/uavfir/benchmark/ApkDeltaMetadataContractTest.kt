...
class ApkDeltaMetadataContractTest {
    ...
    fun parse_bindsCandidateApkRuntimeAndCurrentSourceBuiltNcnnBridgeIdentities() {
        ...
        val parsed = ApkDeltaMetadata.parse(metadata, testManifest(), "8".repeat(64))
    ...
    fun parse_rejectsCandidateSelfReportFromDifferentBridgeSource() {
        val sha = "1".repeat(64)
        val runtime = "2".repeat(64)
        val bridge = "3".repeat(64)
        val metadata = """
            {"abi":"arm64-v8a","modelCandidatesSha256":"${"a".repeat(64)}","baselineApkBytes":100,
             "candidates":{
               "onnx":{"apkBytes":120,"apkDeltaBytes":20,"provenance":{"apkSha256":"${"4".repeat(64)}","runtimeEntries":[{"path":"lib/arm64-v8a/libonnxruntime4j_jni.so","sha256":"$sha"}],"modelEntries":[]}},
               "tflite":{"apkBytes":120,"apkDeltaBytes":20,"provenance":{"apkSha256":"${"5".repeat(64)}","runtimeEntries":[{"path":"lib/arm64-v8a/libtensorflowlite_jni.so","sha256":"$sha"}],"modelEntries":[]}},
               "ncnn":{"apkBytes":120,"apkDeltaBytes":20,"provenance":{"apkSha256":"${"6".repeat(64)}","runtimeEntries":[{"path":"lib/arm64-v8a/libncnn.so","sha256":"$runtime"},{"path":"lib/arm64-v8a/libfire_detector_ncnn.so","sha256":"$bridge"}],"modelEntries":[],"ncnnBuild":{"version":"$APPROVED_NCNN_VERSION","packageArchiveSha256":"$APPROVED_NCNN_ARCHIVE_SHA256","bridgeSourceSha256":"${"8".repeat(64)}","runtimeSha256":"$runtime","bridgeSha256":"$bridge"}}}
             }}
        """.trimIndent()

        ApkDeltaMetadata.parse(metadata, testManifest(), "9".repeat(64))
    }
}
