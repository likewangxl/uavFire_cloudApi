...
export class UranusMqtt extends EventEmitter {
  ...
  waitForConnected = (timeoutMs = 5000) => {
    ...
    return new Promise<void>((resolve, reject) => {
      const timer = window.setTimeout(() => {
        this.off('onStatus', handleStatus)
        reject(new Error('MQTT connect timeout.'))
      }, timeoutMs)
      const handleStatus = (statusOptions: any) => {
        if (statusOptions?.status === 'open') {
        ...
        if (statusOptions?.status === 'error') {
          window.clearTimeout(timer)
          this.off('onStatus', handleStatus)
          reject(statusOptions.data || new Error('MQTT connect failed.'))
        }
      }
      this.on('onStatus', handleStatus)
    })
  }
  ...
  _onConnect = () => {
    window.console.info('mqtt connect,')
    this.emit('onStatus', {
      status: 'open',
      retryCount: 0,
    })
