...
function firmwareStatusText (text: string) {
  if (text === 'All') return '全部'
  if (text === 'Disabled') return '禁用'
  if (text === 'Available') return '可用'
  return text
}
...
function beforeUpload (file: FileItem) {
  if (!file.name || !file.name?.endsWith('.zip')) {
    message.error('文件格式错误，请选择 zip 文件。')
    return false
  }
  fileList.value = [file]
  return false
...
const uploadFile = async () => {
  ...
  formRef.value.validate().then(async () => {
    ...
    uploading = (file.name ?? 'uploading') + timestamp
    notification.open({
      key: uploading,
    ...
    importFirmareFile(workspaceId, fileData).then((res) => {
      if (res.code === 0) {
        notification.success({
          ...
          description: `[${file.name}] 上传成功，耗时：${moment.duration(new Date().getTime() - timestamp).asSeconds()} 秒`,
          duration: null
        })
        getAllFirmwares(pageParam)
      } else {
