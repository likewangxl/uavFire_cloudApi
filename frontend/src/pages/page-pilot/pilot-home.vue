...
const deviceQueryInFlight = ref(false)
let thingConnectCheckTimer: any

function normalizeMqttHost (mqttAddr: string): string {
  if (!mqttAddr) {
    return ''
  }
  return mqttAddr.replace(/^tcp:\/\//i, '').replace(/^mqtt:\/\//i, '')
}

function buildThingCandidates (mqttAddr: string): string[] {
  const normalizedHost = normalizeMqttHost(mqttAddr)
  const candidates = [normalizedHost]
  if (normalizedHost) {
    candidates.push(`tcp://${normalizedHost}`)
  }
  return [...new Set(candidates.filter(Boolean))]
}

function clearThingConnectCheckTimer () {
  if (thingConnectCheckTimer) {
    clearTimeout(thingConnectCheckTimer)
    thingConnectCheckTimer = null
  }
}

function installThingComponent (mqttAddr: string, mqttUsername: string, mqttPassword: string) {
  clearThingConnectCheckTimer()
  const hostCandidates = buildThingCandidates(mqttAddr)
  let currentIndex = 0
  ...
  const tryInstallThing = () => {
    ...
    const param: ThingParam = {
      host: hostCandidates[currentIndex],
...
onMounted(() => {
  ...
  getUserInfo().then(res => {
    ...
    installThingComponent(res.data.mqtt_addr, res.data.mqtt_username, res.data.mqtt_password)
...
function getDeviceInfo () {
  if (deviceQueryInFlight.value || thingState.value !== EStatusValue.CONNECTED) {
    return
  }
  if (!device.data.sn || device.data.sn === EStatusValue.DISCONNECT) {
    return
  }
  deviceQueryInFlight.value = true
  getDeviceBySn(bindParam.workspace_id, device.data.sn).then(res => {
    ...
    deviceQueryInFlight.value = false
  })
}
</script>
