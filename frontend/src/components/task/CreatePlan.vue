  ...
  <div class="create-plan-wrapper">
    ...
    <div class="content">
      <a-form ref="valueRef" layout="horizontal" :hideRequiredMark="true" :rules="rules" :model="planBody" labelAlign="left">
        ...
        <!-- 任务类型 -->
        <a-form-item label="巡检类型" class="plan-timer-form-item">
        ...
        <template v-if="planBody.task_type === TaskType.Condition">
          <!-- battery capacity -->
          <a-form-item label="飞行器电量达到后启动" :labelCol="{span: 23}" name="min_battery_capacity">
          ...
          <!-- storage capacity -->
          <a-form-item label="机场/飞行器剩余存储不低于（MB）" :labelCol="{span: 23}" name="storage_capacity">
            <a-input-number v-model:value="planBody.min_storage_capacity" class="width-100">
            </a-input-number>
          </a-form-item>
        </template>
        <!-- RTH Altitude Relative to Dock -->
        <a-form-item label="返航高度（相对机场，米）" :labelCol="{span: 23}" name="rth_altitude">
...
const rules = {
  name: [
    ...
    { max: 20, message: '计划名称长度不能超过 20 个字符' }
  ],
  file_id: [{ required: true, message: '请选择巡检航线' }],
  dock_sn: [{ required: true, message: '请选择执行设备' }],
  select_execute_time: [{
