...
> - 历史 IP 切换记录仅作背景；仓库当前统一在 `172.20.10.7`（本机 wifi）
...
- RC Plus logcat 持续报：
  - `java.net.UnknownServiceException: CLEARTEXT communication to 172.20.10.7 not permitted by network security policy`
...
**事实更正**：历史 IP 切换记录仅作背景。仓库当前的真实统一基准是 `172.20.10.7`（Mac 当前 wifi 接口 en0），具体见：

- `backend/uavfire/src/main/resources/application.yml`（MQTT host、pilot2 web-entry、livestream.playback.webrtc-host、RTMP、GB28181、WHIP 全部为 `172.20.10.7`）
- `frontend/src/api/http/config.ts` + `frontend/env/.env`（baseURL / websocketURL / rtmpURL 全部为 `172.20.10.7`）
- `rcplus-msdk-agent/gradle.properties`（`agentBackendBaseUrl` / `agentMediaHost` 为 `172.20.10.7`）
- `deployment/zlmediakit/.env`、`deployment/zlmediakit/config/config.ini`（`ZLM_PUBLIC_HOST` / `externIP` 为 `172.20.10.7`）

三方推流 / 信令路径在 `172.20.10.7` 上依然完整对齐：

- Agent RTMP publish：`rtmp://172.20.10.7:1935/live/{droneSn}-0`
- Backend 兜底播放地址：`webrtc://172.20.10.7:58925/live/{droneSn}-0`
- 驾驶舱向 ZLM 发的信令：`http://172.20.10.7:58925/index/api/webrtc?app=live&stream={droneSn}-0&type=play`
