...
```bash
curl -fsS -o /dev/null -w '%{http_code}\n' http://192.168.0.12:6789/                          # 期望 302
curl -fsS -o /dev/null -w '%{http_code}\n' http://192.168.0.12:8080/                          # 期望 200
curl -fsS http://192.168.0.12:58925/index/api/getServerConfig                                   # 期望返回 JSON（即便 Please login first 也算 ZLM 在跑）
```

URL 三方对齐（仓库当前实际配置）：
- agent RTMP publish：`rtmp://192.168.0.12:1935/live/{droneSn}-0`
- backend 兜底 play URL：`webrtc://192.168.0.12:58925/live/{droneSn}-0`
- 驾驶舱 ZLM signaling：`http://192.168.0.12:58925/index/api/webrtc?app=live&stream={droneSn}-0&type=play`
...
```bash
redis-cli -h 192.168.0.12 GET 'dual-stream:group:RC_PLUS_LOCAL'
```
...
```json
{
  ...
  "visiblePlayUrl": "webrtc://192.168.0.12:58925/live/RC_PLUS_LOCAL-0"
}
```
...
```bash
curl -fsS 'http://192.168.0.12:58925/index/api/getMediaList?schema=rtmp' | jq '.data[] | {app, stream}'
```
...
- agent 侧 `BuildConfig.AGENT_MEDIA_HOST` 当前是 `192.168.0.12`（来自 `gradle.properties`），`AGENT_MEDIA_RTMP_PORT=1935`，`AGENT_MEDIA_STREAM_APP=live`
- ZLM 是 SSH 端口转发上来的，确认 SSH 进程没断：`lsof -nP -iTCP:1935 -sTCP:LISTEN`

## 3. 驾驶舱出画

浏览器打开 `http://192.168.0.12:8080/`，点导航 "驾驶舱"，点 "直播画面" tab。
...
- 或者 ZLM 的 SDP 候选地址带的是错的 IP（`externIP=192.168.0.12` 要对上当前 wifi）
...
- ZLM 配置：`deployment/zlmediakit/.env`（`ZLM_PUBLIC_HOST=192.168.0.12`）+ `deployment/zlmediakit/config/config.ini`（`externIP=192.168.0.12`）

## 5. 出问题不要做的事

- **不要切 IP**：当前所有配置都已经统一在 192.168.0.12，乱改会把链路再断一遍
