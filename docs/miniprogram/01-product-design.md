...
- 不把代码、构建、SDK 支持清单或接口可调用等同于任何机型的真机与实飞验收。

### 1.5 多机型产品边界

- 当前 M300 工作分支只是实现起点，产品按大疆行业机型族提供统一体验。
- “M3”必须明确为 M3E、M3T、M3M、M3D 或 M3TD；消费级 Mavic 3 不自动纳入。
- M3E/M3T、M30/M30T、M300/M350、M3D/M3TD、M4E/M4T 进入首批适配矩阵；M4D/M4TD、M400 和后续官方行业机型通过版本化适配器扩展。
- 领导看到统一的任务、风险、报告和审批视图；底层 RC、Dock、内置相机、外挂负载差异由服务端适配器处理。
- 未识别或未验收机型显示“仅查看”，不显示可执行的航线或飞行控制按钮。
