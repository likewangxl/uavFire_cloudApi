...
### F5 TC.2 递进/环绕/上风（在 F2 基础上验证，建议 F2 直接用 TC.2 版本跑）

- **目的**：验证两段递进（第二段朝激光坐标飞）、离散环绕交会、上风向接近。
- **步骤**：同 F2，观察完整作业；有明显风（≥2m/s）的天气各跑一次有风/无风场景。
- **观察**：`FireConfirmProcessor` 日志——`laser fix ... leg=1`（第一段激光坐标）、第二段 flyTo 目标是否为该坐标、`orbit point skipped/visited`、环绕各点激光样本、最终上报坐标 = orbitFix。
- **通过判据**：①第二段悬停位明显朝第一段激光坐标修正（对照地图）；②环绕 3 点至少访问 2 点、orbitFix 与单点 fix 差值记录（预期 ≤5m）；③有风时接近方向在上风侧（对照现场风向）——**若方向反了（跑到下风侧吃烟），说明 MSDK 风向语义与假设相反，把 `DjiWindProvider` 的来向/去向换算取反即可**，这是标注过的待验证项；④全流程时长记录（预期 3–5min），评估是否要减环绕点数。
- **回退**：现场新策略异常时配置 `approachLegsM=[60.0]` + `orbitBearingCount=0` 即回到 F2 的单跳行为（有测试锁死等价性），无需重新打包。
...
| 递进段距 [100,60]m / 环绕半径 60m / 点数 3 | 经验值 | F5 用第一段激光修正量与环绕散布定；时长超预算先减点数 |
| 风速阈值 2m/s / 风向语义 | 假设值 | F5 有风场景验证接近侧；KeyWindSpeed 单位为 dm/s（已在代码换算） |
