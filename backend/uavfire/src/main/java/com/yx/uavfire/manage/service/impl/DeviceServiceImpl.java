...
import com.yx.uavfire.common.error.CommonErrorEnum;
...
import com.yx.uavfire.manage.model.enums.PropertySetFieldEnum;
import com.yx.uavfire.manage.model.enums.UserTypeEnum;
import com.yx.uavfire.manage.model.param.DeviceQueryParam;
import com.yx.uavfire.manage.model.receiver.BasicDeviceProperty;
...
public class DeviceServiceImpl implements IDeviceService {
    ...
    public void pushDeviceOfflineTopo(String workspaceId, String deviceSn) {
        if (!StringUtils.hasText(workspaceId)) {
            log.debug("Skip pushing offline topology because workspaceId is empty. deviceSn={}", deviceSn);
            return;
        }
    ...
    public void pushDeviceOnlineTopo(String workspaceId, String gatewaySn, String deviceSn) {
        if (!StringUtils.hasText(workspaceId)) {
            log.debug("Skip pushing online topology because workspaceId is empty. gatewaySn={}, deviceSn={}",
                    gatewaySn, deviceSn);
            return;
        }
        webSocketMessageService.sendBatch(
    ...
    public void pushOsdDataToPilot(String workspaceId, String sn, DeviceOsdHost data) {
        if (!StringUtils.hasText(workspaceId)) {
            log.debug("Skip pushing pilot OSD because workspaceId is empty. sn={}", sn);
            return;
        }
        webSocketMessageService.sendBatch(
    ...
    public void pushOsdDataToWeb(String workspaceId, BizCodeEnum codeEnum, String sn, Object data) {
        if (!StringUtils.hasText(workspaceId)) {
            log.debug("Skip pushing web OSD because workspaceId is empty. sn={}, code={}", sn, codeEnum);
            return;
        }
        webSocketMessageService.sendBatch(
